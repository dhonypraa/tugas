...
class User extends CI_Controller
{
	...
	public function index()
	{
		...
		if($this->session->userdata('level') == "Admin"){
			...
			//print_r($this->model_app->user()->result());
			$object = array();
	        foreach ($this->model_app->user()->result() as $value) {
			...
			//print_r($data['dd_pegawai'] = $this->model_app->dropdown_pegawai());
			$data['id_pegawai'] = "";
			//print_r($data['dd_pegawai']);
	...
	public function tambah()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$config = Array(  
				'protocol'  => 'smtp',  
				'smtp_host' => 'ssl://smtp.localhost.com',  
				'smtp_port' => 465,  
				'smtp_user' => 'administrator@localhost.com',   
				'smtp_pass' => '123456',   
				'mailtype'  => 'html',   
				'charset'   => 'iso-8859-1'  
			);
			...
			$this->email->from('administrator@localhost.com', 'Helpdesk');   
			$this->email->to($email);   
