...
class Model_app extends CI_Model
{
	...
	public function approve($id)
	{
		...
		$datatracking = array(
			...
			'deskripsi'  => encryptAES_vigenere("Prioritas tiket ditetapkan " . $prio . " dan ditugaskan ke teknisi."),
			'id_user'    => $id_user
		);
	...
	public function input_tugas($id)
	{
		...
		$datatracking = array(
			'id_ticket'  => $id,
			'tanggal'    => date("Y-m-d  H:i:s"),
			'status'     => "Tiket diberikan ke teknisi",
			'deskripsi'  => "",
			'id_user'    => $id_user
		);
	...
	public function update($id)
	{
		...
		if (!$this->upload->do_upload('fileupdate')) {
			$this->session->set_flashdata('status', 'Ada yang salah! File gambar lebih dari 2MB atau format tidak didukung');
			...
			if ($progress == 100) {
				if (date("Y-m-d  H:i:s") > $sql->deadline) {
					...
					$datatracking = array(
						'id_ticket'  => $id,
						'tanggal'    => $date,
						'status'     => "Tiket Ditutup. Progres: " . $progress . " %",
						'deskripsi'  => encryptAES_vigenere(ucfirst($this->input->post('desk'))),
						'id_user'    => $id_user,
	...
	public function changekategori($id)
	{
		...
		if ($subkat == $sql->id_sub_kategori) {
			$this->session->set_flashdata('status1', 'Anda belum mengubah sub kategori!');
			...
			if (!$this->upload->do_upload('filediagnosa')) {
				$this->session->set_flashdata('status', 'Ada yang salah! File lebih dari 2MB atau format tidak didukung');
				redirect('List_ticket_tek/change/' . $id);
			} else {
				$gambar = $this->upload->data();
				$data = array(
				...
				$datatracking = array(
					'id_ticket'  => $id,
					'tanggal'    => date("Y-m-d  H:i:s"),
					'status'     => "Kategori Berubah menjadi " . $row->nama_kategori . "(" . $key->nama_sub_kategori . ")",
	...
	public function getTek()
	{
		$query = $this->db->query("select A.id_user, B.nama, SUM(C.status NOT IN (1,2,6,7)) as total FROM user A 
                                    ...
                                    WHERE A.level = 'technician' GROUP BY B.nama,A.id_user ");
		return $query;
	}
	...
	public function line_bulan()
	{
		$query = $this->db->query("SELECT MONTHNAME(tanggal) AS bulan, COUNT(*) AS jumbulan FROM ticket 
                                   ...
                                   ORDER BY MONTHNAME(tanggal) ASC");
